...
class TypesupHmmInitializationTests {
  ...
  def test_em_uniform() {
    ...
    assertEquals(1.0 / (0 + 3), em("dog", "default"), 1e-5)
    assertEquals(1.0 / (0 + 3), em("runs", "default"), 1e-5)
    assertEquals(1.0 / (0 + 3), em("chases", "default"), 1e-5)
    assertEquals(1.0 / (0 + 3), em("default", "default"), 1e-5)
  ...
  def test_InterpolatingTransitionInitializer {
    ...
    val sentences1 = Vector[Vector[String]]()
    val td1 = SimpleTagDictionary[String, Cat](Map(), "<S>", AtomCat("<S>"), "<E>", AtomCat("<E>"), Set(),
      Set(n, s / s, s \ s, n / n, n \ n, s \ n, (s \ n) / n))

    val init = new InterpolatingTransitionInitializer(Vector(
      new TransitionInitializer[String, Cat] {
        override def apply(
          sentences: Vector[Vector[String]],
          initialTagdict: TagDictionary[String, Cat] //
          ): ConditionalProbabilityDistribution[Cat, Cat] = {
          assert(sentences eq sentences1)
          assert(initialTagdict eq td1)
    ...
    val tr = init(sentences1, td1)
    assertEquals((0.6 * 0.11 + 0.4 * 0.41), tr(n, s), 1e-9)
    assertEquals((0.6 * 0.31 + 0.4 * 0.61), tr(s, n / n), 1e-9)
  }

  @Test
  def test_em_TagDictEstimate() {
    val sentences = Vector(
      "a cat chases the dog",
      "the dog walks",
      "the man walks the dog",
    ...
    val em = new EmTagDictEstimate(
      new TagDictEstimateTagPriorInitializer,
      0.2, 1.0, false)(sentences, tagdict)
    assertEquals(1.0, em("<S>", "<S>"), 1e-5)
    assertEquals(0.0, em("the", "<S>"), 1e-5)
    assertEquals(0.0, em("a", "<S>"), 1e-5)
    assertEquals(0.0, em("cat", "<S>"), 1e-5)
  ...
  @Test
  def test_em_TagDictEstimate_manualTagPrior() {
    ???
  }
